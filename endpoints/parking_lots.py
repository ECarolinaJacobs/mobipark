...
def get_parking_lot_by_id(parking_lot_id: str):
    ...
    lot = next((lot for lot in parking_lots if lot.get("id") == parking_lot_id), None)
    if lot is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Parking lot does not exist")

    return lot


@router.get(
    "/parking-lots/", summary="Retrieve all current parking lots", response_description="Parking lot details"
)
...
@router.post("/parking-lots/", summary="Create new parking lot", response_description="Parking lot creation")
def create_parking_lot(
    ...
    return JSONResponse(content=new_lot, status_code=status.HTTP_200_OK)


@router.post("/parking-lots/{parking_lot_id}/sessions/start")
def start_parking_session(
    ...
    return JSONResponse(content=started_session, status_code=status.HTTP_200_OK)


@router.put("/parking-lots/{parking_lot_id}/sessions/stop")
def stop_parking_session(
    ...
    return JSONResponse(content=stopped_session, status_code=status.HTTP_200_OK)
...
def update_parking_lot(
    ...
    return JSONResponse(content=updated_lot, status_code=status.HTTP_200_OK)
...
def update_parking_session(
    ...
    auth_services.verify_admin(session_user)
    updated_session = parking_services.update_parking_session(
        parking_lot_id, parking_session_id, parking_session_data
    )

    return JSONResponse(content=updated_session, status_code=status.HTTP_200_OK)
