...
from utils.session_calculator import generate_payment_hash, generate_transaction_validation_hash
...
def get_payment_by_id(payment_id: str, session_user: Dict[str, str] = Depends(require_auth)) -> JSONResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to load payment data"
        )
    ...
    if not (is_owner or is_admin):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Cannot access payments that are not your own"
        )
...
def get_all_payments(session_user: Dict[str, str] = Depends(require_auth)) -> JSONResponse:
    """
...
def create_payment(
    payment_create: PaymentCreate, session_user: Dict[str, str] = Depends(require_auth)
) -> JSONResponse:
    """
    ...
    try:
        ...
        if not payment_create.t_data:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Transaction data (t_data) is required"
            )
        if payment_create.amount < 0 or (
            payment_create.t_data.amount is not None and payment_create.t_data.amount < 0
        ):
        ...
        if payment_create.discount_code:
            try:
                ...
                if not discount.get("active", True):
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST, detail="Discount code is not active"
                    )
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to apply discount code"
                )
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save payment"
            )

        logger.info(f"Payment created: {transaction_hash} by {session_user['username']}")

        return JSONResponse(content=payment, status_code=status.HTTP_201_CREATED)
...
def update_payment(
    payment_id: str, payment_update: PaymentUpdate, session_user: Dict[str, str] = Depends(require_auth)
) -> JSONResponse:
    """
    ...
    try:
        ...
        if session_user["role"] != ROLE_ADMIN:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permissions to update payments"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save payment update"
            )
