...
from models.refunds_model import RefundCreate, Refund, RefundUpdate, DiscountCodeCreate, DiscountCode
...
def create_refund(
    refund_create: RefundCreate, session_user: Dict[str, str] = Depends(require_admin)
) -> JSONResponse:
    """
    ...
    try:
        ...
        total_refunded = sum(
            refund["amount"] for refund in existing_refunds if refund.get("status") == "completed"
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save refund"
            )

        logger.info(
            f"Refund created: {refund_id} for transaction {refund_create.original_transaction_id} by {session_user['username']}"
        )

        return JSONResponse(content=refund, status_code=status.HTTP_201_CREATED)
...
def get_refund_by_id_endpoint(
    refund_id: str, session_user: Dict[str, str] = Depends(require_auth)
) -> JSONResponse:
    """
...
def get_all_refunds(session_user: Dict[str, str] = Depends(require_auth)) -> JSONResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to load refunds"
        )
...
def get_refunds_for_transaction(
    transaction_id: str, session_user: Dict[str, str] = Depends(require_auth)
) -> JSONResponse:
    """
    ...
    try:
        ...
        if not original_payment:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Transaction {transaction_id} not found"
            )
...
def create_discount_code(
    discount_create: DiscountCodeCreate, session_user: Dict[str, str] = Depends(require_admin)
) -> JSONResponse:
    """
...
def get_all_discount_codes(session_user: Dict[str, str] = Depends(require_admin)) -> JSONResponse:
    """
...
def get_discount_code_by_code(
    code: str, session_user: Dict[str, str] = Depends(require_admin)
) -> JSONResponse:
    """
...
def update_discount_code(
    code: str, discount_update: DiscountCodeCreate, session_user: Dict[str, str] = Depends(require_admin)
) -> JSONResponse:
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update discount code"
            )
