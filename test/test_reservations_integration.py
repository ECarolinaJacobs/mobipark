...
def test_create_reservation_as_admin(vehicle_creation_succes_admin):
    ...
    data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1", "user_id": USER_LOGIN["username"]}
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
...
def test_missing_vehicle_id():
    create_user(False)
    headers = get_session()
    data = {"start_time": "2025-12-06T10:00Z", "parking_lot_id": "1"}
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
...
def test_incorrect_vehicle_id_format():
    create_user(False)
    headers = get_session()
    data = {"vehicle_id": "1233456", "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1"}
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
...
def test_parking_lot_id_not_found_user(vehicle_creation_succes):
    vehicle, headers = vehicle_creation_succes
    data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "99999"}
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
...
def test_parking_lot_id_not_found_admin(vehicle_creation_succes_admin):
    vehicle, headers = vehicle_creation_succes_admin
    data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "99999", "user_id": "test"}
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
...
def test_update_missing_vehicle_id(vehicle_creation_succes):
    ...
    new_data = {"start_time": "2025-12-06T10:00Z", "parking_lot_id": "1"}
...
def test_update_cost_as_user(vehicle_creation_succes):
    ...
    reservation_id = res.json()["reservation"]["id"]
    new_data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1", "cost": "4567890"}
    res_put = requests.put(f"{URL}/reservations/{reservation_id}", json=new_data, headers=headers)
...
def test_update_status_as_user(vehicle_creation_succes):
    ...
    reservation_id = res.json()["reservation"]["id"]
    new_data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1", "status": "confirmed" }
    res_put = requests.put(f"{URL}/reservations/{reservation_id}", json=new_data, headers=headers)
...
def test_update_invalid_status_as_admin(vehicle_creation_succes_admin):
    vehicle, headers = vehicle_creation_succes_admin
    data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1", "user_id" : vehicle["user_id"]} 
    res = requests.post(f"{URL}/reservations/", json=data, headers=headers)
    assert res.status_code == 201
    
    reservation_id = res.json()["reservation"]["id"]
    new_data = {"vehicle_id": vehicle["id"], "start_time": "2025-12-06T10:00Z", "parking_lot_id": "1", "status": "invalid_status", "user_id" : vehicle["user_id"] }
    res_put = requests.put(f"{URL}/reservations/{reservation_id}", json=new_data, headers=headers)
    assert res_put.status_code == 403
...
def test_update_reservation_success(vehicle_creation_succes):
    ...
    reservation_id = res.json()["reservation"]["id"]
    new_data = {"vehicle_id": vehicle["id"],"start_time": "2025-12-06T10:00Z", "parking_lot_id": "2"}
    res_put = requests.put(f"{URL}/reservations/{reservation_id}", json=new_data, headers=headers)
