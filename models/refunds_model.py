...
class RefundCreate(BaseModel):
    ...
    amount: float = Field(
        ..., ge=0, description="Amount to refund (must be positive and <= original payment amount)"
    )
    ...
    def validate_amount(cls, v):
        if v <= 0:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Refund amount must be positive"
            )
        return v
